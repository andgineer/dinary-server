...
отсутствие отравленных (poisoned) задач логгинга. Возвращает ненулевой код при любом сбое.

Отравленная задача логгинга терминальна — расход не попадёт в таблицу, пока строку
не вернут в очередь:

```bash
inv requeue-sheet-jobs --prod   # poisoned -> pending, дрейн подхватит на следующем проходе
inv requeue-sheet-jobs          # то же для локальной data/dinary.db
```
